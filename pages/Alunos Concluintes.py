...
# ---------- Filtrar apenas os alunos que concluíram ----------
df = df[df["SituacaoAlunoAgrupada"] == "Conclusão"]
...
st.title("🎓 Alunos Concluintes por Ano")
...
# ---------- GRÁFICO GERAL: Concluintes por Ano ----------
df_ano = df_filtrado.groupby("AnoAdmissao").size().reset_index(name="Concluintes")
...
fig_ano = px.bar(
    ...
    x="AnoAdmissao",
    y="Concluintes",
    title="Total de alunos concluintes por ano",
    labels={"AnoAdmissao": "Ano", "Concluintes": "Total de Concluintes"},
    height=600
)
fig_ano.update_layout(xaxis_title="Ano de Admissão", yaxis_title="Concluintes")
