...
# ---------- Filtrar apenas os alunos evadidos ----------
df = df[df["SituacaoAlunoAgrupada"] == "Evasão"]
...
st.title("🎓 Alunos com Evasão por Ano")
...
# ---------- GRÁFICO GERAL: Evasões por Ano ----------
df_ano = df_filtrado.groupby("AnoAdmissao").size().reset_index(name="Evasões")
...
fig_ano = px.bar(
    ...
    x="AnoAdmissao",
    y="Evasões",
    title="Total de alunos com evasão por ano",
    labels={"AnoAdmissao": "Ano", "Evasões": "Total de Evasões"},
    height=600
)
fig_ano.update_layout(xaxis_title="Ano de Admissão", yaxis_title="Evasões")
