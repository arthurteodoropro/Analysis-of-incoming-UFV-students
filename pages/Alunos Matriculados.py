...
# ---------- Filtrar apenas os alunos matriculados ----------
df = df[df["SituacaoAlunoAgrupada"] == "Matriculado"]
...
st.title("🎓 Alunos Matriculados por Ano")
...
# ---------- GRÁFICO GERAL: Matriculados por Ano ----------
df_ano = df_filtrado.groupby("AnoAdmissao").size().reset_index(name="Matriculados")
...
fig_ano = px.bar(
    ...
    x="AnoAdmissao",
    y="Matriculados",
    title="Total de alunos matriculados por ano",
    labels={"AnoAdmissao": "Ano", "Matriculados": "Total de Matriculados"},
    height=600
)
fig_ano.update_layout(xaxis_title="Ano de Admissão", yaxis_title="Matriculados")
